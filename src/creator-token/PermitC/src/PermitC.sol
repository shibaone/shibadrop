...
contract PermitC is Ownable, CollateralizedPausableFlags, EIP712, IPermitC {
     ...
     */
    mapping(bytes32 => mapping(address => PackedApproval))
        private _transferApprovals;
    ...
    constructor(
        ...
        CollateralizedPausableFlags(_nativeValueToCheckPauseState)
        EIP712(name, version)
    {
    ...
    modifier onlyRegisteredTransferAdvancedTypeHash(
    ...
    function updateApprovalBySignature(
        ...
        // current block.
        approvalExpiration = approvalExpiration == 0
     ...
     */
    function registerAdditionalDataHash(
        string calldata additionalDataTypeString
    ) external {
    ...
    function permitTransferFromWithAdditionalDataERC721(
        ...
        external
        onlyRegisteredTransferAdvancedTypeHash(advancedPermitHash)
     ...
     */
    function isRegisteredTransferAdditionalDataHash(bytes32 hash)
        external
        view
     ...
     * @return isRegistered true if the hash is valid, false otherwise
     */
    function isRegisteredOrderAdditionalDataHash(bytes32 hash)
        external
        view
        returns (bool isRegistered)
    {
    ...
    function fillPermittedOrderERC1155(
        ...
        external
        onlyRegisteredOrderAdvancedTypeHash(advancedPermitHash)
        returns (uint256 quantityFilled, bool isError)
    {
        ...
        (quantityFilled, isError) = _orderTransfer(
            orderStatus,
            orderFillAmounts,
            token,
            id,
    ...
    function closePermittedOrder(
        ...
        _requireValidTokenType(tokenType);
        PackedApproval storage orderStatus = _getPackedApprovalPtr(
            _orderApprovals,
            owner,
            tokenType,
            token,
     ...
     */
    function isValidUnorderedNonce(address owner, uint256 nonce)
        external
        view
        returns (bool isValid)
    {
        isValid =
            ((_unorderedNonces[owner][uint248(nonce >> 8)] >> uint8(nonce)) &
                ONE) ==
            ZERO;
    }
    ...
    function transferFromERC721(
        ...
        PackedApproval storage approval = _checkAndUpdateApproval(
            owner,
            TOKEN_TYPE_ERC721,
            token,
            id,
    ...
    function _transferFromERC721(
        ...
        if (!isError) {
            try IERC721(token).transferFrom(owner, to, id) {} catch {
                isError = true;
            }
        }
    }
    ...
    function _ecdsaRecover(
        ...
        bytes32 r,
        bytes32 vs
    ) private pure returns (bool isError, address signer) {
        unchecked {
    ...
    function _safeIsValidSignature(
        ...
        assembly {
            function _callIsValidSignature(
                ...
                let dataLength := add(
                    0x64,
                    and(add(_signatureLength, 0x1F), not(0x1F))
                )
                ...
                if and(
                    iszero(lt(returndatasize(), 0x20)),
                    staticcall(gas(), _signer, ptr, dataLength, 0x00, 0x20)
                ) {
     ...
     */
    function _requireTransferAdvancedPermitHashIsRegistered(
        bytes32 advancedPermitHash
    ) private view {
     ...
     * @param advancedPermitHash  The permit typehash to check
     */
    function _requireOrderAdvancedPermitHashIsRegistered(
        bytes32 advancedPermitHash
    ) private view {
    ...
    function _checkAndInvalidateNonce(address account, uint256 nonce) private {
        unchecked {
            if (
                uint256(
                    _unorderedNonces[account][uint248(nonce >> 8)] ^= (ONE <<
                        uint8(nonce))
                ) &
                    (ONE << uint8(nonce)) ==
                ZERO
    ...
    function _checkAndUpdateApproval(
        ...
        approval = _getPackedApprovalPtr(
            _transferApprovals,
            owner,
            tokenType,
            token,
    ...
    function _getPackedApprovalPtr(
        ...
        approval = _approvals[
            _getPackedApprovalKey(account, tokenType, token, id, orderId)
        ][operator];
    }
     ...
     */
    function _getPackedApprovalKey(
        address owner,
        uint256 tokenType,
        address token,
        uint256 id,
    ...
    function _storeApproval(
        ...
        PackedApproval storage approval = _getPackedApprovalPtr(
            _transferApprovals,
            owner,
            tokenType,
            token,
    ...
    function _checkOrderTransfer(
        ...
        orderStatus = _getPackedApprovalPtr(
            _orderApprovals,
            owner,
            tokenType,
            token,
    ...
    function _allowance(
        ...
        allowedAmount = allowed.expiration < block.timestamp
            ? 0
            : allowed.amount;
     ...
     */
    function _requireCallerHasPausePermissions()
        internal
        view
        virtual
        override
