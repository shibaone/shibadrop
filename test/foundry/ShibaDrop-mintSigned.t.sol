...
contract ShibaDropMintSignedTest is TestHelper {
    using ECDSA for bytes32;
    ERC721ShibaDrop token2;
    ...
    function setUp() public {
        ...
        token2.updateCreatorPayoutAddress(address(shibadrop), creator);
        ...
        validateFuzzInputsSigners(args)
    {
        ...
        shibadrop.updateSignedMintValidationParams(
            signer,
            signedMintValidationParams
        );
        shibadrop.updateAllowedFeeRecipient(args.feeRecipient, true);
        vm.stopPrank();

        // Allow the payer.
        token.updatePayer(address(shibadrop), args.payer, true);

        hoax(args.payer, 100 ether);

        // Calculate the value to send with the transaction.

        shibadrop.mintSigned{ value: args.numMints * mintParams.mintPrice }(
            address(token),
            args.feeRecipient,
            args.minter,
            args.numMints,
    ...
    function testMintSigned_validSignatureWrongToken(
        ...
        // Allow the payer.
        token.updatePayer(address(shibadrop), args.payer, true);
        token2.updatePayer(address(shibadrop), args.payer, true);

        // Calculate the value to send with the transaction.
        uint256 mintValue = args.numMints * mintParams.mintPrice;

        hoax(args.payer, 100 ether);
        shibadrop.mintSigned{ value: mintValue }(
            address(token),
            args.feeRecipient,
            args.minter,
            args.numMints,
    ...
    function testMintSigned_unknownSigner(string memory signerSeed) public {
        ...
        shibadrop.mintSigned{ value: mintParams.mintPrice }(
            address(token),
            feeRecipient,
            msg.sender,
            1,
    ...
    function testMintSigned_differentPayerThanMinter(
        ...
        shibadrop.updateAllowedFeeRecipient(feeRecipient, true);
        vm.stopPrank();
        ...
        shibadrop.mintSigned{ value: mintParams.mintPrice * 2 }(
            address(token),
            feeRecipient,
            minter,
            2,
