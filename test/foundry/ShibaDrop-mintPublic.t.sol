...
import {
    INonFungibleShibaDropToken
} from "shibadrop/interfaces/INonFungibleShibaDropToken.sol";

import { PublicDrop } from "shibadrop/lib/ShibaDropStructs.sol";

import { MaliciousRecipient } from "shibadrop/test/MaliciousRecipient.sol";

contract ERC721ShibaDropMintPublicTest is TestHelper {
    ...
    function setUp() public {
        ...
        token.updateCreatorPayoutAddress(address(shibadrop), creator);
        ...
        shibadrop.updatePublicDrop(publicDrop);
    }
    ...
    function testMintPublicReenter() public payable {
        ...
        assertEq(
            uint256(
                shibadrop.getPublicDrop(address(token)).maxTotalMintableByWallet
            ),
            1
        );
        ...
        attacker.attack(shibadrop, address(token));
    }

    function testMintPublic(FuzzInputs memory args) public validateArgs(args) {
        PublicDrop memory publicDrop = shibadrop.getPublicDrop(address(token));
        ...
        shibadrop.mintPublic{ value: mintValue }(
            address(token),
            args.feeRecipient,
            args.minter,
            args.numMints
        ...
        validateArgs(args)
    {
        ...
        shibadrop.mintPublic{ value: 1 wei }(
            address(token),
            args.feeRecipient,
            args.minter,
            args.numMints
        ...
        validateArgs(args)
    {
        ...
        // Allow the payer.
        token.updatePayer(address(shibadrop), payer, true);
    ...
    function testMintSeaDrop_revertNonShibaDrop(FuzzInputs memory args)
        public
        validateArgs(args)
    {
        vm.expectRevert(
