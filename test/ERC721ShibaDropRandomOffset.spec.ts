...
import type {
  ERC721ShibaDropRandomOffset,
  IShibaDrop,
} from "../typechain-types";
import type { Wallet } from "ethers";

describe(`ERC721ShibaDropRandomOffset (v${VERSION})`, function () {
  const { provider } = ethers;
  let shibadrop: IShibaDrop;
  let token: ERC721ShibaDropRandomOffset;
  let owner: Wallet;
  let creator: Wallet;
  let minter: Wallet;
  ...
  beforeEach(async () => {
    // Deploy token
    const ERC721ShibaDropRandomOffset = await ethers.getContractFactory(
      "ERC721ShibaDropRandomOffset",
      owner
    );
    token = await ERC721ShibaDropRandomOffset.deploy("", "", [
      shibadrop.address,
    ]);
  });
