...
import type { TokenGatedDropStageStruct } from "../typechain-types/src/ShibaDrop";
import type { Wallet } from "ethers";

describe(`ShibaDrop - Mint Allowed Token Holder (v${VERSION})`, function () {
  ...
  beforeEach(async () => {
    ...
    token = await SeaDropToken.deploy("", "", [shibadrop.address]);
  ...
  it("Should mint a token to a user with the allowed NFT token", async () => {
    ...
    expect(
      await shibadrop.getAllowedNftTokenIdIsRedeemed(
        token.address,
  ...
  it("Should mint a token to a user with the allowed NFT token when the payer is different from the minter", async () => {
    ...
    await token
      .connect(owner)
      .updatePayer(shibadrop.address, owner.address, true);
  ...
  it("Should revert if the drop stage is not active", async () => {
    ...
    await expect(
      shibadrop.mintAllowedTokenHolder(
        token.address,
        feeRecipient.address,
  ...
  it("Should not mint an allowed token holder stage with a different token contract", async () => {
    ...
    // Deploy a new ERC721ShibaDrop.
    const SeaDropToken = await ethers.getContractFactory("ERC721ShibaDrop");
  ...
  it("Should not be able to set an allowedNftToken to the drop token itself or zero address", async () => {
    await expect(
      token.updateTokenGatedDrop(shibadrop.address, token.address, dropStage)
  ...
  it("Should not be able to set an invalid fee bps", async () => {
    await expect(
      token.updateTokenGatedDrop(shibadrop.address, allowedNftToken.address, {
        ...dropStage,
        feeBps: 15_000,
      })
  ...
  it("Should clear from enumeration when deleted", async () => {
    await token
      .connect(owner)
      .updateTokenGatedDrop(shibadrop.address, allowedNftToken.address, {
        ...dropStage,
        maxTotalMintableByWallet: 0,
      });
    expect(
    ...
    expect(
      await shibadrop.getTokenGatedDrop(token.address, allowedNftToken.address)
