...
import type { SignedMintValidationParamsStruct } from "../typechain-types/src/ERC721ShibaDrop";
import type { MintParamsStruct } from "../typechain-types/src/ShibaDrop";
import type { Wallet } from "ethers";

describe(`ShibaDrop - Mint Signed (v${VERSION})`, function () {
  ...
  before(async () => {
    ...
    eip712Domain = {
      ...
      verifyingContract: shibadrop.address,
    };
    eip712Types = {
      SignedMint: [
  ...
  it("Should not mint a signed mint with different params", async () => {
    ...
    const value = BigNumber.from(mintParams.mintPrice).mul(3);
    await expect(
      shibadrop.connect(payer).mintSigned(
        token.address,
        feeRecipient.address,
    ...
    const token2 = await ERC721ShibaDrop.deploy("", "", [shibadrop.address]);
    await token2.setMaxSupply(100);
    await token2.updateCreatorPayoutAddress(shibadrop.address, creator.address);
    ...
    expect(
      await shibadrop.getSignedMintValidationParams(
        token.address,
        signer2.address
      )
    ...
    await expect(
      shibadrop.connect(minter).mintSigned(
