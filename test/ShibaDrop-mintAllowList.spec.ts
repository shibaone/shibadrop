...
describe(`ShibaDrop - Mint Allow List (v${VERSION})`, function () {
  const { provider } = ethers;
  let shibadrop: IShibaDrop;
  let token: INonFungibleShibaDropToken;
  let creator: Wallet;
  let owner: Wallet;
  let minter: Wallet;
  ...
  it("Should mint a free mint allow list stage", async () => {
    ...
    expect(await shibadrop.getAllowListMerkleRoot(token.address)).to.eq(root);
  ...
  it("Should mint an allow list stage with a different payer than minter", async () => {
    ...
    // Allow the payer.
    await token.updatePayer(shibadrop.address, owner.address, true);
